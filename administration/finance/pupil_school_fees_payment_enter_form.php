<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        if (empty($session) || empty($term) || empty($class) || empty($voucher_num)) {
            ...
            header("location: pupil_school_fees_payment_form.php?fail=$fail");
            exit();
            ...
            if (!preg_match($session_reg, $session)) {
                ...
                if ($num < 1) {
                    ...
                    if ($num_two < 1) {
                        ...
                        $query_three = "SELECT * FROM pupil_registration_table WHERE addmission_num = '$addmission_num' AND status = 'active'";
                        $query_run_three = mysqli_query($conn, $query_three);

                        $num_three = mysqli_num_rows($query_run_three);
                        
                        if ($num_three < 1) {
                            $fail = 'pupil not active';
                            header("location: pupil_school_fees_payment_form.php?fail=$fail");
                            exit();
                        }else {
...
<?php
    ...
    $page_title = 'pupils school fees payment form';
...
?>
    ...
    <div id="form_container">
        ...
        <div id="header">
            <h2>pupils school fees payment</h2>
            
            <div id="img">
                <img src="../../image/pupil/<?php echo $image ?>" alt="" srcset="">
            </div>
        </div>
            ...
            $('#submit').click(function (event) {
               ...
               if (amount == '' ) {
                    ...
                    $.ajax({
                        ...
                        data: {action: 'pupil school fees payment', class: classe, term, session, voucher_num, addmission_num, amount},
                        method: 'POST',
                        dataType: 'text',

<?php
    ...
    if (!isset($_POST['submit'])) {
        ...
        header("location: school_fees_details_form.php");
        exit();
    }
    ...
    if ($term === '' || $academic_session === '') {
        ...
        header("location: school_fees_details_form.php?fail=" . urlencode('fill all the inputs'));
        exit();
    }

    if (!preg_match("/^([0-9]{4})\/([0-9]{4})$/", $academic_session)) {

        header("location: school_fees_details_form.php?fail=" . urlencode('academic session format is incorrect'));
        exit();
    }
    ...
    if ($conn) {
        ...
        $amount_deposited = principal_sum($conn, 'school_deposit_transaction_table', 'amount', "term = '$term_sql' AND session = '$session_sql'");
        $amount_withdraw = principal_sum($conn, 'school_withdraw_transaction_table', 'amount', "term = '$term_sql' AND session = '$session_sql' AND status = 'approved'");

        $amount_expected = 0;
        $amount_generated = 0;

        $vouchers = principal_rows($conn, "SELECT class, total FROM class_voucher_table WHERE term = '$term_sql' AND session = '$session_sql'");
...
<div class="context-bar">
    ...
    <a href="school_fees_details_form.php">Change</a>

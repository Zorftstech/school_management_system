<?php
    ...
    if (!isset($_POST['submit'])) {
        ...
        header("location: withdraw_details_form.php");
        exit();
    }
    ...
    if ($term === '' || $academic_session === '') {
        ...
        header("location: withdraw_details_form.php?fail=" . urlencode('fill all the inputs'));
        exit();
    }

    if (!preg_match("/^([0-9]{4})\/([0-9]{4})$/", $academic_session)) {

        header("location: withdraw_details_form.php?fail=" . urlencode('academic session format is incorrect'));
        exit();
    }
    ...
    if ($conn) {
        ...
        $rows = principal_rows($conn, "SELECT * FROM school_withdraw_transaction_table WHERE session = '$session_sql' AND term = '$term_sql' ORDER BY id DESC");
    ...
    $page_title = 'College withdrawals';
...
<div class="context-bar">
    ...
    <a href="withdraw_details_form.php">Change</a>
        ...
        $('.not_approve_btn').click(function(event){
            ...
            if (confirm('do you want to approve this transaction?')) {
                ...
                $.ajax({
                    ...
                    data: {action: 'appove withdraw transaction', id},
                    method: 'POST',
                    dataType: 'text',

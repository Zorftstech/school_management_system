                ...
                if (!preg_match($session_reg, $academic_session)) {
                    ...
                    if (!in_array($image_exten, $allow)) {
                        ...
                        // }else{
                            ...
                            $addmission_array = array($addmission_year, 'p', substr(uniqid('', true), 18));
                            $addmission_num = implode('/', $addmission_array);
                           

                            $query = "INSERT INTO pupil_registration_table (surname, first_name, other_name, date_birth, gender, nationality, age, state, local_govt, old_school, start_class, disability, health_issue, session, image, f_surname, f_first_name, f_other_name, f_phone_number, f_email, f_address, home_town, religion, furture_career, game, best_three_subject, reg_date, status, addmission_num, current_class, pwd) VALUES ('$surname', '$first_name', '$other_name', '$data_birth', '$gender', '$country', '$age', '$state', '$local_government', '$old_school', '$start_class', '$disability', '$health', '$academic_session', '$image_pass', '$f_surname', '$f_first', '$f_other', '$f_phone', '$f_email', '$f_address', '$home_town', '$religion', '$furture_career', '$game', '$best_three_subject', '$reg_date', '$status', '$addmission_num', '$start_class', '$pwd')";

                            $query_run = mysqli_query($conn, $query);

                            if ($query_run) {
                                $out = 'you are successfully registered';
                                header("location: ../pupil_registration_form.php?correct=$out");
                            }else{

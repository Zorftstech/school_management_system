<?php
     ...
     if (isset($_POST['action'])) {
        ...
        if ($_POST['action'] == 'formaster login') {
            ...
            $query = "SELECT * FROM student_attendance_creation_table WHERE email = '$email' AND term = '$term' AND user_name = '$user' AND session = '$session' AND pwd = '$pwd'";
            ...
            if ($num < 1) {
                ...
                if ($attendance_status != 'open') {
                    ...
                    if ($email_status != 'verified') {
                        ...
                        $query_two = "UPDATE student_attendance_creation_table SET id_code = '$id_code' WHERE email = '$email' AND term = '$term' AND user_name = '$user' AND session = '$session' AND pwd = '$pwd'";
        ...
        if ($_POST['action'] == 'formaster idcode verification') {
            ...
            $query = "SELECT * FROM student_attendance_creation_table WHERE email = '$email' AND id_code = '$id_code' AND term = '$term' AND session = '$session' AND email_status = 'verified' AND attendance_status = 'open'";
            $query_run = mysqli_query($conn, $query);
        ...
        if ($_POST['action'] == 'formaster reset password') {
            ...
            $query = "SELECT * FROM student_attendance_creation_table WHERE pwd_code = '$code' AND pwd_token = '$token'";
            $query_run = mysqli_query($conn, $query);
            ...
            if ($num > 0) {
                ...
                $query_two = "UPDATE student_attendance_creation_table SET pwd = '$pwd' WHERE pwd_code = '$code' AND pwd_token = '$token'";
                $query_run_two = mysqli_query($conn, $query_two);
